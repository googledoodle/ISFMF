...
class DDF_module():
    ...
    def __init__(self, output_dimesion, vocab_size, image_size, dropout_rate, emb_dim, max_len, nb_filters):
        ...
        deepContour_model.add(Conv2D(32, 5, strides=(1, 1), activation='relu',
                              padding='same', input_shape=(image_size, image_size, 3), name="conv1"))
        deepContour_model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2)))
        deepContour_model.add(Conv2D(48, 5, strides=(
            1, 1), activation='relu', padding='same', name="conv2"))
        deepContour_model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2)))
        deepContour_model.add(Conv2D(64, 5, strides=(
            1, 1), activation='relu', padding='same', name="conv3"))
        deepContour_model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2)))
        deepContour_model.add(Conv2D(128, 5, strides=(
            1, 1), activation='relu', padding='same', name="conv4"))
        deepContour_model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2)))
